...
use crate::{
    ...
    storage::{tests::utils::create_mask, Storage},
};
...
pub fn compose_update_message() -> Message {
    let payload = Update {
        sum_signature: ParticipantTaskSignature::zeroed(),
        update_signature: ParticipantTaskSignature::zeroed(),
        masked_model: create_mask(1, 1),
...
pub fn send_update_messages(n: u32, request_tx: RequestSender) {
    for _ in 0..n {
        let request = request_tx.clone();
        tokio::spawn(async move { request.msg(&compose_update_message()).await });
    }
}
